...
##    def Read_db(self):
##        self.Verify_presence_of_network()
...
##            self.Create_new_database()
##            self.db_opened = True
##            # Recond that network is based on database with filename db_name
##            self.Gel_net.db_name = self.db_name
...
if main.use_GUI:
    ...
    while action != "s":
        if action == 'c':
            main.Gel_net = Semantic_Network(self.net_name)
            main.Create_net()
